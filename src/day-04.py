...
def p1(input_data):
    ...
    for card_data in input_data:
        ...
        winning_numbers = [int(winning_numbers_str[i:i+3]) for i in range(0, len(winning_numbers_str), 3)]
        card_numbers = [int(card_numbers_str[i:i+3]) for i in range(0, len(card_numbers_str), 3)]
...
def p2(input_data):
    ...
    print(f"The total number of card copies is {sum(card_copies)}")




with open("day-04-puzzle-input.txt", "r") as f:
